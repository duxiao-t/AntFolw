...
import type { MobileHistoryItem } from './processes.api';
...
export function ProcessSnapshotTimeline({
  history,
  processSnapshot,
}: {
  ...
  return (
    <ol style={listStyle} aria-label="流程快照进度">
      {history.map((entry) => (
...
function nodeLabel(snapshot: unknown, nodeId?: string | null): string {
  ...
  return findNodeName(snapshot, nodeId) ?? nodeId;
}
...
export default ProcessSnapshotTimeline;
