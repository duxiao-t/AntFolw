...
验证环境使用 Windows 原生 PostgreSQL 17，数据库名 `antflow`；并非 Docker PostgreSQL。验证时曾遇到后端重启瞬间的 `INTERNAL_ERROR`，服务完全启动后重试成功。
...
2. 没有在全新 Docker PostgreSQL 数据库上完整执行并验证 V15-V17 迁移。当前只在本机原生 PostgreSQL 17 上验证了 V17 和现有数据修复。
...
7. `README.md`、`docs/mobile-enterprise-verification.md`、`codex.md`、`agent.md` 中仍有旧的迁移版本、Docker/原生 PostgreSQL 假设或旧设计差异说明，尚未统一更新。
...
- 本地同时存在原生 PostgreSQL 和 Docker 配置，端口占用曾导致连接到陈旧数据库，验证结果需要结合实际进程和数据库确认。
...
1. 在干净 Docker PostgreSQL 17 上执行完整 Flyway 迁移，确认 V15/V16/V17 可重复部署、约束和历史修复数据正确。
